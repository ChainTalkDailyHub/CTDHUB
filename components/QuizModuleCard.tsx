...
export default function QuizModuleCard({ module, isCompleted, isLocked }: QuizModuleCardProps) {
  const getCardClass = () => {
    let baseClass = "card transition-all duration-300 "
    
    if (isLocked) {
      baseClass += "opacity-60 cursor-not-allowed"
    } else if (isCompleted) {
      baseClass += "border-green-500 shadow-green-500/20"
    } else {
      baseClass += "hover:shadow-lg hover:shadow-primary/20 border-primary/50"
    }
  ...
  return (
    <div className={getCardClass()}>
      ...
      <p className="text-sm text-gray-400 mb-6">{module.questions.length} questions</p>
